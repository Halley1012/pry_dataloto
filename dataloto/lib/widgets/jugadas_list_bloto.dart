...
class JugadasListBloto extends StatefulWidget {
  final AnimationController jugadasController;

  const JugadasListBloto({super.key, required this.jugadasController});

  @override
  JugadasListBlotoState createState() => JugadasListBlotoState();
}

class JugadasListBlotoState extends State<JugadasListBloto>
  ...
  Future<void> _fetchJugadas() async {
    try {
      final jugadas = await ApiService.listarJugadasBloto();
      if (!mounted) return;
      setState(() {
  ...
  Future<void> addJugada(List<int> numeros, String userId, {String? fechaSorteo}) async {
    ...
    try {
      final newJugada = await ApiService.crearJugadaBloto(numeros, userId, fechaSorteo: fechaSorteo);
      if (!mounted) return;
      setState(() {
  ...
  Future<void> deleteJugada(int jugadaId) async {
    ...
    // Buscar jugada antes de modificar el estado
                          ...
                          child: Row(
                            ...
                            children: [
                                ...
                                child: Row(
                                  ...
                                  children: numeros.isEmpty
                                      ...
                                      : numeros.asMap().entries.map((entry) {
                                          ...
                                          bool isSuperBall =
                                              ballIndex == numeros.length - 1;

                                          Widget ball = _build3DBall(
                                            numero,
                                            baseColor: isSuperBall
                                                ? Colors.redAccent
                                                : color,
                                            size: 35,
                                          );
                              ...
                              GestureDetector(
                                ...
                                child: const Icon(
                                  Icons
                                      .delete_forever_rounded, // 🔥 mismo estilo que Miloto
