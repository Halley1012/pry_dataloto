...
class _MegaMillionsScreenState extends State<MegaMillionsScreen> with TickerProviderStateMixin {
  ...
  Future<void> _cargarDataOptimizado() async {
    ...
    final cached = await CacheService.getJson('megamillions_prediccion');
    ...
    try {
      ...
      debugPrint("❌ Error al cargar Mega Millions: $e");
    } finally {
  ...
  Future<void> _fetchUltimosResultados() async {
    try {
      final response = await http.get(Uri.parse("https://pry-dataloto.onrender.com/megamillions/ultimos5"));
      if (response.statusCode == 200) {
        ...
        if (data["resultados"] != null && mounted) {
          ...
          CacheService.setJson('megamillions_ultimos5', data);
        }
      }
    } catch (_) {}
  }
  ...
  Future<void> _fetchHistoricoCompleto() async {
    final cached = await CacheService.getJson('megamillions_historico_completo');
    ...
    try {
      final response = await http.get(Uri.parse("https://pry-dataloto.onrender.com/megamillions/historico_completo"));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        if (data["resultados"] != null && mounted) {
          setState(() {
          ...
          CacheService.setJson('megamillions_historico_completo', data);
        }
      }
    } catch (_) {}
  }
  ...
  Future<void> fetchNumeros() async {
    try {
      ...
      if (response.statusCode == 200 && mounted) {
        ...
        CacheService.setJson('megamillions_prediccion', data);
      }
    } catch (e) {
  ...
  Future<void> _guardarJugada(AppLocalizations? l10n) async {
    ...
    if (!isCustomMode) {
      ...
      if (seleccionados.length != 5 || balotaRojaSeleccionada == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l10n?.debesSeleccionarBalotas ?? "Debes seleccionar 5 balotas principales y 1 Mega Ball roja")),
        );
        return;
      }
      ...
      children: [
        Column(
          ...
          children: [
            Row(
              children: [
                const LotteryAvatar3D(nombre: "Mega Millions", size: 32),
                const SizedBox(width: 10),
                Text("Mega Millions", style: AppTextStyles.tituloPrincipal.copyWith(fontSize: 18)),
              ],
            ),
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(_jackpot ?? "\$250", style: AppTextStyles.h2.copyWith(color: AppColors.yellow, fontWeight: FontWeight.bold, fontSize: 15)),
  ...
  Widget _buildIAPrediction(AppLocalizations? l10n) {
    ...
    final int scoreAfinidad = _calcularAfinidadScore(numsEvaluados, 70);
      ...
      children: [
        ...
        _buildActionTile(
          ...
          onTap: () => Navigator.push(
            context,
            MaterialPageRoute(builder: (_) => const EstadisticasMegaMillionsScreen()),
          ),
        ),
      ],
    );
                    ...
                    return GridView.count(
                      ...
                      children: listaProbables.asMap().entries.map((entry) {
                        ...
                        Color baseColor = isSelected
                            ? Colors.amber
                            : (index < 35 ? Colors.redAccent : const Color(0xFF607D8B));
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(l10n?.balotasRojas ?? "Mega Ball Roja", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
            ],
          ),
          const SizedBox(height: 4),
