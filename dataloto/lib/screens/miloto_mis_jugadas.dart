...
class MilotoMisJugadasScreen extends StatefulWidget {
  ...
  @override
  State<MilotoMisJugadasScreen> createState() => _MilotoMisJugadasScreenState();
}
...
class _MilotoMisJugadasScreenState extends State<MilotoMisJugadasScreen> {
  ...
  void _toggleSelectAll() {
    setState(() {
      if (_selectedIds.length == _jugadasList.length && _jugadasList.isNotEmpty) {
        _selectedIds.clear();
      } else {
  ...
  Future<void> _eliminarSeleccionadas() async {
    ...
    int eliminadas = 0;
    final idsAEliminar = Set<int>.from(_selectedIds);

    for (final id in idsAEliminar) {
      if (_userId != null) {
        final ok = await ApiService.borrarJugadaMloto(id, _userId!);
        if (ok) eliminadas++;
      }
    }
                                ...
                                return Container(
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      // 3. Balotas compactas (5 Números de MiLoto)
                                      Expanded(
                                        child: Row(
