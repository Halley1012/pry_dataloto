...
class _MillionaireLifeScreenState extends State<MillionaireLifeScreen> with TickerProviderStateMixin {
  ...
  static const int totalBalotas = 58;
  static const int totalEspeciales = 5;
  ...
  Future<void> _cargarDatos() async {
    ...
    try {
      ...
      debugPrint("❌ Error cargando Millionaire for Life: $e");
    } finally {
  ...
  Future<void> _fetchPrediccion() async {
    try {
      ...
      debugPrint("❌ Error al obtener predicción de Millionaire for Life: $e");
    }
  }
  ...
  Future<void> _guardarJugada() async {
    ...
    if (balotaEspecialSeleccionada == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Debes seleccionar 1 Cash Ball roja')),
      );
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('✅ ¡Jugada de Millionaire for Life guardada exitosamente!')),
      );
      _fetchJugadas();
    } catch (e) {
      if (!mounted) return;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            "Selecciona 5 balotas blancas (1-$totalBalotas) y 1 Cash Ball (1-$totalEspeciales):",
          ...
          const SizedBox(height: 16),
          Text("Cash Ball (1-$totalEspeciales):", style: const TextStyle(color: Colors.white70)),
          const SizedBox(height: 8),
          GridView.builder(
            shrinkWrap: true,
            physics: const NeverScrollableScrollPhysics(),
