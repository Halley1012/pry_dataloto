...
class _MillionaireLifeScreenState extends State<MillionaireLifeScreen> with TickerProviderStateMixin {
  ...
  Future<void> _cargarOptimizado() async {
    ...
    if (cached != null) setState(() { listaProbables = List<int>.from(cached["numeros"]); fechaPrediccion = cached["fecha"]; });
    try {
  ...
  Future<void> fetchNumeros() async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (mounted) setState(() { listaProbables = List<int>.from(data["numeros"]); fechaPrediccion = data["fecha"]; });
              ...
              child: Padding(
                ...
                child: Column(children: [
                  AppContainer(child: Column(children: [
                    Text("Predicciones Inteligentes IA", style: AppTextStyles.tituloPrincipal.copyWith(color: Colors.amberAccent)),
                    Text("Sorteo: ${fechaPrediccion ?? '...'}", style: AppTextStyles.mensajeSecundario),
                  ])),
                  const SizedBox(height: 20),
                  AppContainer3(child: Column(children: [
                    Text("Tu combinación sugerida", style: AppTextStyles.h2.copyWith(color: Colors.amberAccent)),
                    ...
                    Row(children: [
                      ...
                      Expanded(child: ElevatedButton(onPressed: () => Navigator.pushNamed(context, '/estadisticas_millionaire_life'), style: ElevatedButton.styleFrom(backgroundColor: AppColors.yellow, foregroundColor: Colors.black, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30))), child: const Text("Estadísticas"))),
                    ]),
                    const SizedBox(height: 10),
                    ElevatedButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => LoteriasMisJugadasScreen(loteria: loteria, route: backendRoute))), style: ElevatedButton.styleFrom(backgroundColor: AppColors.yellow, foregroundColor: Colors.black, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30))), child: const Text("Mis jugadas")),
                  ])),
                  const SizedBox(height: 20),
  ...
  Widget _buildResultadosSection() {
    return AppContainer3(child: Column(children: [
      ...
      if (mostrarResultados) ...ultimosResultados.map((r) => Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("${r['fecha']}", style: AppTextStyles.mensajeImportante), Row(children: (r['numeros'] as List).map((n) => Padding(padding: const EdgeInsets.only(left: 4), child: _buildBall(n as int, size: 28))).toList())]))),
    ]));
  }

  Widget _buildBall(int n, {double size = 40}) => Container(width: size, height: size, decoration: BoxDecoration(color: Colors.amber, shape: BoxShape.circle, boxShadow: [BoxShadow(blurRadius: 4, offset: Offset(2, 2))]), child: Center(child: Text("$n", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black, fontSize: size * 0.4))));
}
