...
class _MillionaireLifeScreenState extends State<MillionaireLifeScreen> with TickerProviderStateMixin {
  static const String loteria = 'Millionaire Life';
  ...
  Future<void> _cargarDataOptimizado() async {
    ...
    final cached = await CacheService.getJson('millionaire_life_prediccion');
    ...
    try {
      ...
      debugPrint("❌ Error al cargar Millionaire Life: $e");
    } finally {
  ...
  Future<void> _fetchUltimosResultados() async {
    try {
      final response = await http.get(Uri.parse("https://pry-dataloto.onrender.com/millionaire_life/ultimos5"));
      if (response.statusCode == 200) {
        ...
        if (data["resultados"] != null && mounted) {
          ...
          CacheService.setJson('millionaire_life_ultimos5', data);
        }
      }
    } catch (_) {}
  }
  ...
  Future<void> _fetchHistoricoCompleto() async {
    final cached = await CacheService.getJson('millionaire_life_historico_completo');
    ...
    try {
      final response = await http.get(Uri.parse("https://pry-dataloto.onrender.com/millionaire_life/historico_completo"));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        if (data["resultados"] != null && mounted) {
          setState(() {
          ...
          CacheService.setJson('millionaire_life_historico_completo', data);
        }
      }
    } catch (_) {}
  }
  ...
  Future<void> fetchNumeros() async {
    try {
      ...
      if (response.statusCode == 200 && mounted) {
        ...
        CacheService.setJson('millionaire_life_prediccion', data);
      }
    } catch (e) {
  ...
  Future<void> _guardarJugada(AppLocalizations? l10n) async {
    ...
    if (!isCustomMode) {
      ...
      if (seleccionados.length != 5 || balotaRojaSeleccionada == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(l10n?.debesSeleccionarBalotas ?? "Debes seleccionar 5 balotas principales y 1 balota roja")),
        );
        return;
      }
  ...
  Widget _buildIAPrediction(AppLocalizations? l10n) {
    ...
    final int scoreAfinidad = _calcularAfinidadScore(numsEvaluados, 58);
                    ...
                    return GridView.count(
                      ...
                      children: listaProbables.asMap().entries.map((entry) {
                        ...
                        Color baseColor = isSelected
                            ? Colors.amber
                            : (index < 29 ? Colors.redAccent : const Color(0xFF607D8B));
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(l10n?.balotasRojas ?? "Cash Ball Roja", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
            ],
          ),
          const SizedBox(height: 4),
