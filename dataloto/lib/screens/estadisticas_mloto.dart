...
class EstadisticasMlotoScreen extends StatefulWidget {
  ...
  @override
  State<EstadisticasMlotoScreen> createState() => _EstadisticasMlotoScreenState();
}

class _EstadisticasMlotoScreenState extends State<EstadisticasMlotoScreen> {
  bool cargando = true;
  String? errorMensaje;
  ...
  Future<void> _cargarDatos() async {
    ...
    try {
      final resHist = await http.get(
        Uri.parse("https://pry-dataloto.onrender.com/mloto/historico_completo"),
      );
      final resPred = await http.get(
        Uri.parse("https://pry-dataloto.onrender.com/mloto"),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          l10n?.estadisticas ?? "Estadísticas MiLoto",
  ...
  Widget _buildFiltrosBarra(AppLocalizations? l10n) {
    return AppContainer3(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text("${l10n?.filtrosAnalisis ?? "Filtros de Análisis"} (MiLoto)", style: AppTextStyles.h2),
    ...
    return AppContainer3(
      child: Row(
        ...
        children: [
          ...
          _buildMetricStat("Rango Balotas", "1 - 39"),
          _buildMetricStat(l10n?.tipo ?? "Combinación", "5 Números"),
        ],
      ),
    );
  }
  ...
  Widget _buildCardGraficaFrecuencia(List<Map<String, dynamic>> resultados, AppLocalizations? l10n) {
    ...
    final String titleText = "1. ${l10n?.frecuenciaHistorica ?? "Frecuencia Histórica"} (Balotas 1 - 39)";
      ...
      return SizedBox(
        ...
        child: BarChart(
          BarChartData(
            ...
            barGroups: List.generate(39, (idx) {
              final ball = idx + 1;
    ...
    return AppContainer3(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text("${l10n?.balotasMayorTiempo ?? "Balotas con mayor tiempo sin aparecer:"} (MiLoto)", style: AppTextStyles.caption),
          const SizedBox(height: 12),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceAround,
  ...
  Widget _buildCardBajosAltos(List<Map<String, dynamic>> resultados, AppLocalizations? l10n) {
    ...
    final String titleText = "5. ${l10n?.bajosAltos ?? "Bajos (1-20) vs Altos (21-39)"}";
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _buildLegendItem("${l10n?.bajosAltos.split("vs").first.trim() ?? "Bajos (1-20)"}: $bajos ($pctBajos%)", Colors.cyanAccent),
                const SizedBox(height: 12),
                _buildLegendItem("${l10n?.bajosAltos.split("vs").last.trim() ?? "Altos (21-39)"}: $altos ($pctAltos%)", Colors.purpleAccent),
              ],
            ),
          ),
        ],
  ...
  Widget _buildCardSumaCombinaciones(List<Map<String, dynamic>> resultados, AppLocalizations? l10n) {
    ...
    for (var r in resultados.take(30)) {
      final nums = List<int>.from(r["numeros"] ?? []);
      final s = nums.fold(0, (acc, val) => acc + val);
      if (s > 0) sumas.add(s.toDouble());
    }

    final reversedSumas = sumas.reversed.toList();
    final avgSuma = sumas.isEmpty ? 100 : (sumas.reduce((a, b) => a + b) / sumas.length).toStringAsFixed(0);
  ...
  Widget _buildCardParejasYTrios(List<Map<String, dynamic>> resultados, AppLocalizations? l10n) {
    ...
    for (var r in resultados) {
      final nums = List<int>.from(r["numeros"] ?? [])..sort();
  ...
  Widget _buildCardScoreIA(List<Map<String, dynamic>> resultados, AppLocalizations? l10n) {
    final score = 91; // Score algorítmico / IA para MiLoto
    return AppContainer3(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          const SizedBox(height: 8),
          Text("${l10n?.indiceAfinidadHistorica ?? "Índice de afinidad estadística"} MiLoto:", style: AppTextStyles.caption),
          const SizedBox(height: 16),
          Row(
            children: [
              Expanded(
          ...
          const SizedBox(height: 8),
          Text(l10n?.altaConsistenciaIA ?? "✅ Alta frecuencia en rango medio-alto y equilibrio par/impar optimizado.",
    ...
    return AppContainer3(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text("${l10n?.prediccionActualIA ?? "Predicción actual del modelo IA"} (MiLoto):", style: AppTextStyles.caption),
          const SizedBox(height: 6),
          Wrap(
            spacing: 6,
            runSpacing: 6,
  ...
  Map<int, int> _calcularAusencias(List<Map<String, dynamic>> resultados) {
    ...
    for (int i = 1; i <= 39; i++) ausencias[i] = 999;
