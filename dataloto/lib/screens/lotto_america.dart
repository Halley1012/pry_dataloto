...
class _LottoAmericaScreenState extends State<LottoAmericaScreen> with TickerProviderStateMixin {
  ...
  Future<void> _cargarDataOptimizado() async {
    ...
    final cached = await CacheService.getJson('lotto_america_prediccion');
    ...
    try {
      ...
      debugPrint("❌ Error al cargar Lotto America: $e");
    } finally {
  ...
  Future<void> _fetchUltimosResultados() async {
    try {
      final response = await http.get(Uri.parse("https://pry-dataloto.onrender.com/lotto_america/ultimos5"));
      if (response.statusCode == 200) {
        ...
        if (data["resultados"] != null && mounted) {
          ...
          CacheService.setJson('lotto_america_ultimos5', data);
        }
      }
    } catch (_) {}
  }
  ...
  Future<void> _fetchHistoricoCompleto() async {
    final cached = await CacheService.getJson('lotto_america_historico_completo');
    ...
    try {
      final response = await http.get(Uri.parse("https://pry-dataloto.onrender.com/lotto_america/historico_completo"));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        if (data["resultados"] != null && mounted) {
          setState(() {
          ...
          CacheService.setJson('lotto_america_historico_completo', data);
        }
      }
    } catch (_) {}
  }
  ...
  Future<void> fetchNumeros() async {
    try {
      ...
      if (response.statusCode == 200 && mounted) {
        ...
        CacheService.setJson('lotto_america_prediccion', data);
      }
    } catch (e) {
      ...
      children: [
        Column(
          ...
          children: [
            Row(
              children: [
                LotteryAvatar3D(nombre: "Lotto America", size: 32),
                const SizedBox(width: 10),
                Text("Lotto America", style: AppTextStyles.tituloPrincipal.copyWith(fontSize: 18)),
              ],
            ),
        ...
        Container(
          ...
          child: Builder(builder: (context) {
            final parts = PaisHelper.getJackpotParts(_jackpot, fallbackValue: "\$3.5 millones USD");
            return Column(
  ...
  Widget _buildIAPrediction(AppLocalizations? l10n) {
    ...
    final int scoreAfinidad = _calcularAfinidadScore(numsEvaluados, 52);
                    ...
                    return GridView.count(
                      ...
                      children: listaProbables.asMap().entries.map((entry) {
                        ...
                        Color baseColor = isSelected
                            ? Colors.amber
                            : (index < 26 ? Colors.redAccent : const Color(0xFF607D8B));
