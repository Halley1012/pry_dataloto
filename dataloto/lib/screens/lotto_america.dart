...
class _LottoAmericaScreenState extends State<LottoAmericaScreen> with TickerProviderStateMixin {
  static const String loteriaNombre = 'Lotto America';
  static const String backendRoute = 'lotto_america';
  static const int maxSeleccion = 5;
  static const int totalBalotas = 52;
  static const int totalEspeciales = 10;
  ...
  Future<void> _cargarDatos() async {
    ...
    try {
      ...
      debugPrint("❌ Error cargando Lotto America: $e");
    } finally {
  ...
  Future<void> _fetchPrediccion() async {
    try {
      ...
      debugPrint("❌ Error al obtener predicción de Lotto America: $e");
    }
  }
  ...
  Future<void> _guardarJugada() async {
    ...
    if (balotaEspecialSeleccionada == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Debes seleccionar 1 Star Ball roja')),
      );
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('✅ ¡Jugada de Lotto America guardada exitosamente!')),
      );
      _fetchJugadas();
    } catch (e) {
      if (!mounted) return;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            "Selecciona 5 balotas blancas (1-$totalBalotas) y 1 Star Ball (1-$totalEspeciales):",
          ...
          const SizedBox(height: 16),
          Text("Star Ball (1-$totalEspeciales):", style: const TextStyle(color: Colors.white70)),
          const SizedBox(height: 8),
          GridView.builder(
            shrinkWrap: true,
            physics: const NeverScrollableScrollPhysics(),
