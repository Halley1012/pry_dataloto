...
class DoublePlayScreen extends StatefulWidget {
  ...
  @override
  State<DoublePlayScreen> createState() => _DoublePlayScreenState();
}

class _DoublePlayScreenState extends State<DoublePlayScreen> with TickerProviderStateMixin {
  static const String loteria = 'Double Play';
  static const String backendRoute = 'double_play';
  static const String backendUrl = "https://pry-dataloto.onrender.com/double_play";
  ...
  Future<void> _guardarJugada() async {
    ...
    try {
      await ApiService.crearJugadaGenerica(backendRoute, [...seleccionados..sort(), balotaRojaSeleccionada!], userId!);
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("✅ Jugada guardada")));
          ...
          slivers: [
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Column(
                  children: [
                    AppContainer(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text("Predicciones Inteligentes para $loteria", style: AppTextStyles.tituloPrincipal.copyWith(color: Colors.amberAccent)),
                      const SizedBox(height: 8),
                      Text("Números con mayor probabilidad para el sorteo del ${fechaPrediccion ?? '...'}", style: AppTextStyles.mensajeSecundario),
                    ])),
  ...
  Widget _buildNavigationButtons() {
    return Row(children: [
      ...
      const SizedBox(width: 8),
      Expanded(child: ElevatedButton(onPressed: () => Navigator.pushNamed(context, '/estadisticas_double_play'), style: ElevatedButton.styleFrom(backgroundColor: AppColors.yellow, foregroundColor: Colors.black, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30))), child: const Text("Estadísticas"))),
    ]);
  }

  Widget _buildSelectionGrid() {
    return AppContainer3(child: Column(children: [
      Text("Selección por IA", style: AppTextStyles.h2),
      const SizedBox(height: 10),
      GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 7, mainAxisSpacing: 8, crossAxisSpacing: 8), itemCount: listaProbables.length, itemBuilder: (context, i) {
        final n = listaProbables[i]; final isSel = seleccionados.contains(n);
        return GestureDetector(onTap: () => setState(() => isSel ? seleccionados.remove(n) : (seleccionados.length < 5 ? seleccionados.add(n) : null)), child: _buildBall(n, isSel ? Colors.amber : (i < 20 ? Colors.redAccent : Colors.blueGrey)));
      }),
    ]));
  }
  ...
  Widget _buildResultadosSection() {
    return AppContainer3(child: Column(children: [
      ...
      if (mostrarResultados) ...ultimosResultados.map((r) => Padding(padding: const EdgeInsets.symmetric(vertical: 6), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("${r['fecha']}", style: AppTextStyles.mensajeImportante), Row(children: (r['numeros'] as List).map((n) => Padding(padding: const EdgeInsets.only(left: 4), child: _buildBall(n as int, Colors.amber, size: 28))).toList())]))),
    ]));
  }

  Widget _buildBall(int n, Color c, {double size = 40}) => Container(width: size, height: size, decoration: BoxDecoration(color: c, shape: BoxShape.circle, boxShadow: [BoxShadow(blurRadius: 4, offset: Offset(2, 2))]), child: Center(child: Text("$n", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white, fontSize: size * 0.4))));
}
