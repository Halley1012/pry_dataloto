...
class _DoublePlayScreenState extends State<DoublePlayScreen> with TickerProviderStateMixin {
  static const String loteriaNombre = 'Double Play';
  static const String backendRoute = 'double_play';
  static const int maxSeleccion = 5;
  static const int totalBalotas = 69;
  static const int totalEspeciales = 26;
  ...
  Future<void> _cargarDatos() async {
    ...
    try {
      ...
      debugPrint("❌ Error cargando Double Play: $e");
    } finally {
  ...
  Future<void> _fetchPrediccion() async {
    try {
      ...
      debugPrint("❌ Error al obtener predicción de Double Play: $e");
    }
  }
  ...
  Future<void> _guardarJugada() async {
    ...
    if (balotaEspecialSeleccionada == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Debes seleccionar 1 balota Powerball roja')),
      );
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('✅ ¡Jugada de Double Play guardada exitosamente!')),
      );
      _fetchJugadas();
    } catch (e) {
      if (!mounted) return;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text(
            "Balota Roja Powerball Propuesta:",
            style: AppTextStyles.mensajeSecundario.copyWith(color: Colors.white),
          ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            "Selecciona 5 balotas blancas (1-$totalBalotas) y 1 roja (1-$totalEspeciales):",
          ...
          const SizedBox(height: 16),
          Text("Balota Roja Powerball (1-$totalEspeciales):", style: const TextStyle(color: Colors.white70)),
          const SizedBox(height: 8),
          GridView.builder(
            shrinkWrap: true,
            physics: const NeverScrollableScrollPhysics(),
