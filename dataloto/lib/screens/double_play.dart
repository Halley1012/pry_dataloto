...
class _DoublePlayScreenState extends State<DoublePlayScreen> with TickerProviderStateMixin {
  ...
  Future<void> _cargarDataOptimizado() async {
    ...
    final cached = await CacheService.getJson('double_play_prediccion');
    ...
    try {
      ...
      debugPrint("❌ Error al cargar Double Play: $e");
    } finally {
  ...
  Future<void> _fetchUltimosResultados() async {
    try {
      final response = await http.get(Uri.parse("https://pry-dataloto.onrender.com/double_play/ultimos5"));
      if (response.statusCode == 200) {
        ...
        if (data["resultados"] != null && mounted) {
          ...
          CacheService.setJson('double_play_ultimos5', data);
        }
      }
    } catch (_) {}
  }
  ...
  Future<void> _fetchHistoricoCompleto() async {
    final cached = await CacheService.getJson('double_play_historico_completo');
    ...
    try {
      final response = await http.get(Uri.parse("https://pry-dataloto.onrender.com/double_play/historico_completo"));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        if (data["resultados"] != null && mounted) {
          setState(() {
  ...
  Future<void> fetchNumeros() async {
    try {
      ...
      if (response.statusCode == 200 && mounted) {
        ...
        CacheService.setJson('double_play_prediccion', data);
      }
    } catch (e) {
      ...
      children: [
        Column(
          ...
          children: [
            Row(
              children: [
                LotteryAvatar3D(nombre: "Double Play", size: 32),
                const SizedBox(width: 10),
                Text("Double Play", style: AppTextStyles.tituloPrincipal.copyWith(fontSize: 18)),
              ],
            ),
        ...
        Container(
          ...
          child: Builder(builder: (context) {
            final parts = PaisHelper.getJackpotParts(_jackpot, fallbackValue: "\$10 millones USD");
            return Column(
