...
class _HomeScreenState extends State<HomeScreen>
  ...
  void initState() {
    ...
    // CARGAR TODO EN PARALELO CON FUTURE.WAIT
    _cargarPaginaPrincipalOptimizado();
  }

  Future<void> _cargarPaginaPrincipalOptimizado() async {
    if (!mounted) return;
    setState(() {
      isLoading = true;
      cargando = true;
    ...
    try {
      ...
      // 🇨🇴 Si pais_id es null, vacio o 'null', fallback por defecto a "1" (Colombia)
      final paisIdStr = (rawPaisId != null && rawPaisId != 'null' && rawPaisId.isNotEmpty)
          ? rawPaisId
          : "1";
      final paisIdInt = int.tryParse(paisIdStr);

      // 2. Cargar Posts, Anuncios y Loterías en PARALELO con tolerancia a fallos
      ...
      // 🔄 Fallback de conteo de respuestas: si un post viene con 0 (ej. backend Render sin re-desplegar),
      // consultamos getComments(p.id) en paralelo para tener la cifra real.
      final postsConConteo = await Future.wait(
        rawPosts.map((p) async {
          if (p.commentsCount > 0) return p;
          try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Image.asset("assets/images/logo_letras.png", height: 100),
        actions: [
          Padding(
                  ...
                  onSelected: (value) async {
                    ...
                    switch (value) {
                      ...
                      case 3:
                        ...
                        // Construir el mapa 'user' con los datos guardados
                        final user = {
                          'name': allData['name'],
                          'email': allData['email'],
                        ...
                        // Si se actualizó el usuario, refrescar los datos en storage
                        if (updatedUser != null &&
                            ...
                            if (_loterias.length > 4) ...[
                              ...
                              Padding(
                                padding: const EdgeInsets.only(
                                  right: 8.0,
                                ), // ← clave para que no se pegue al borde
                                child: Align(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    child: Row(
                                              crossAxisAlignment:
                                                  CrossAxisAlignment.start,
                                              children: [
                                                // 1. Avatar Circular del usuario (tamaño pequeño compacto)
                                                CircleAvatar(
                                                  radius: 12,
                                                ...
                                                // 2. Contenido e Información del post
                                                Expanded(
                                                  child: Column(
                                                    crossAxisAlignment:
                                                        CrossAxisAlignment.start,
                                                    children: [
                                                      // LÍNEA 1: @userName • hace X d
                                                      Row(
                                                        children: [
                                                          Text(
                                                            "@${post.userName}",
                                                      ...
                                                      // LÍNEA 2: Título del post destacado
                                                      Text(
                                                        post.title,
                                                        style: const TextStyle(
                                                          fontWeight: FontWeight.bold,
                                                      ...
                                                      // LÍNEA 4: Fila de Acciones (Responder y Respuestas)
                                                      Row(
                                                        children: [
                                                          // Botón "Responder"
                                                          InkWell(
                                                            ...
                                                            child: const Padding(
                                                              ...
                                                              child: Row(
                                                                children: [
                                                                  Icon(Icons.reply_outlined, color: Colors.white60, size: 14),
                                                                  SizedBox(width: 3),
                                                                  Text(
                                                                    "Responder",
                                                                    style: TextStyle(
                                                          ...
                                                          // Botón "˅ X respuestas" (SIEMPRE VISIBLE en amarillo)
                                                          InkWell(
                                                            onTap: () => _abrirPostScreen(post),
                                                            borderRadius: BorderRadius.circular(10),
                                                            child: Padding(
                                                ...
                                                // 3. Menú de 3 punticos (⋮) a la derecha (solo visible si es el dueño)
                                                if (isOwner)
