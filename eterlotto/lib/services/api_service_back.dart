...
class ApiService {
  ...
  static Future<List<Map<String, dynamic>>> getCategorias() async {
    ...
    final cached = await CacheService.getStaleJson('categorias_list_cache');
  ...
  static Future<List<Map<String, dynamic>>> getPaises() async {
    ...
    // Fallback caché
    final cached = await CacheService.getStaleJson('paises_list_cache');
    if (cached is List && cached.isNotEmpty) {
      return List<Map<String, dynamic>>.from(cached);
    }
    return [];
  ...
  static Future<List<Map<String, dynamic>>> getCiudadesPorDepartamento({
    ...
    try {
      ...
      // Se usa el último catálogo de ciudades conocido si la red no responde.
    }
