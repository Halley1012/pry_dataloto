...
def ejecutar_duplasena():
    from main_duplasena import main as main_duplasena
    main_duplasena()
...
def enviar_notificacion_exito():
    ...
    msg["Subject"] = "DAG Dupla Sena ejecutado exitosamente"
    msg["From"] = sender
    msg["To"] = receiver

    html = """
    <h3>Ejecución de Dupla Sena (Brasil) finalizada con éxito</h3>
    <p>El proceso de scraping, predicción y notificaciones en <b>main_duplasena.py</b> concluyó correctamente.</p>
    """
...
def enviar_notificacion_error(context):
    ...
    html = f"""
    <h3>⚠️ Alerta de Ejecución en Airflow - Dupla Sena</h3>
...
with DAG(
    ...
    tarea_ejecutar_duplasena = PythonOperator(
        task_id='ejecutar_scraping_y_prediccion_duplasena',
        python_callable=ejecutar_duplasena
    )
    ...
    tarea_ejecutar_duplasena >> tarea_notificar_exito
