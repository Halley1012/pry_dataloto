...
def validar_maismilionaria():
    print("==================================================")
    print("Iniciando Validación Histórica para +Milionária (Brasil)")
    print("Lotería ID: 30 | 6 balotas (1..50) + 2 tréboles (1..6)")
    print("==================================================")
    ...
    with engine.connect() as conn:
        df = pd.read_sql(text("SELECT * FROM resultados_maismilionaria ORDER BY concurso ASC;"), conn)

    if df.empty:
        print("❌ Error: resultados_maismilionaria está vacía.")
        sys.exit(1)
    ...
    # 5. Validar rangos de balotas (1..50 para las 6 balotas, 1..6 para los 2 tréboles)
    ...
    for idx, row in df_real.iterrows():
        ...
        tr1 = int(row['balotaroja'])
        tr2 = int(row['balotaroja2'])
        if not (1 <= tr1 <= 6) or not (1 <= tr2 <= 6):
            errores_trevos += 1
            if errores_trevos <= 3:
                print(f"❌ Error rango tréboles en concurso #{row['concurso']} ({row['fecha']}): [{tr1}, {tr2}]")

    if errores_rango > 0 or errores_unicos > 0 or errores_trevos > 0:
        print(f"❌ Errores en balotas/tréboles: {errores_rango} fuera de rango, {errores_unicos} repetidas, {errores_trevos} tréboles.")
        sys.exit(1)
    else:
        print("✅ Todas las balotas en rango [1..50] (6 únicas) y tréboles en rango [1..6]")

    # 6. Placeholders
    print(f"\nPlaceholders encontrados ({len(df_ph)}):")
    for _, r in df_ph.iterrows():
        print(f"  - Concurso #{r['concurso']} ({r['fecha']} - {r['sorteo']}): Balotas={[r[c] for c in balota_cols]} Tréboles=[{r['balotaroja']}, {r['balotaroja2']}]")
    ...
    try:
        ...
        r = requests.get("https://servicebus2.caixa.gov.br/portaldeloterias/api/maismilionaria", headers=headers, timeout=10)
    ...
    print("\n==================================================")
    print("✅ Validación de +Milionária completada exitosamente!")
    print("==================================================")

if __name__ == "__main__":
    validar_maismilionaria()
