...
class EuromillonesPredictor:
    ...
    def run(self):
        print("🔮 Iniciando Predictor de Euromillones (España - XGBoost)...")
        engine = get_engine()
        ...
        try:
            ...
            print(f"❌ Error consultando resultados Euromillones: {e}")
            return

        if df_all.empty:
            print("⚠️ No hay resultados en la base de datos para entrenar el modelo de Euromillones.")
            return

        df_all["fecha"] = pd.to_datetime(df_all["fecha"])
        fecha_proximo_sorteo = df_all['fecha'].max().strftime('%Y-%m-%d')
        print(f"Próximo sorteo a predecir para Euromillones: {fecha_proximo_sorteo}")
        ...
        # --- Crear columnas binarias para las 12 estrellas ---
        ...
        if df_future.empty:
            print("⚠️ No se encontró la fila con la fecha reciente en Euromillones.")
            return
        ...
        # --- MODELO 2: 2 Estrellas Especiales (1 al 12) ---
        ...
        try:
            ...
            conn.execute(text("""
                ...
                WHERE LOWER(loteria_route) = 'euromillones' 
            ...
            conn.execute(text("""
                ...
                VALUES (
                    (SELECT id FROM loterias WHERE LOWER(route) = 'euromillones' OR LOWER(nombre) = 'euromillones' LIMIT 1),
                    'euromillones',
                    :fecha,
                    :numeros,
                    :balotaroja
            ...
            print(f"✅ Predicción de Euromillones guardada exitosamente en BD para la fecha: {fecha_proximo_sorteo}")
            print(f"Top 15 números principales con mayor probabilidad: {numeros_prediccion[:15]}")
            print(f"Ranking estrellas propuestas: {rojaprediccion}")

        except Exception as e:
            print(f"❌ Error al guardar predicciones de Euromillones en BD: {e}")
        finally:
            conn.close()

if __name__ == "__main__":
    EuromillonesPredictor().run()
