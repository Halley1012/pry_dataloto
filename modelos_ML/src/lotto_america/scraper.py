...
class LottoAmericaScraper:
                ...
                for group in soup_main.find_all(class_="game-detail-group"):
                    ...
                    if title_el and "lotto america" in title_el.get_text().lower():
                        ...
                        if num_el:
                            ...
                            date_el = group.find(class_="game-date")
                            if date_el:
                                fecha_str = date_el.get_text(strip=True)
                            break
                            
                if jackpot:
                    self.update_jackpot(self.engine, "lotto_america", jackpot, fecha_str or datetime.now().strftime("%Y-%m-%d"))
        except Exception as e:
            print(f"⚠️ Error actualizando jackpot para Lotto America: {e}")
    ...
    def _asegurar_placeholder(self, ultima_fecha_real: date):
        ...
        with self.engine.begin() as conn:
            conn.execute(text("""
                DELETE FROM resultados_lotto_america
            ...
            conn.execute(text("""
                INSERT INTO resultados_lotto_america (
                    ...
                    NULL, :loteria_id, 'Lotto America', :fecha,
                    0, 0, 0, 0, 0, 0,
        ...
        print(f"🎯 Placeholder verificado para Lotto America ({prox_fecha_str})")
    ...
    def run(self, max_pages=None):
        ...
        if fuente_info is None:
            raise RuntimeError("❌ [Lotto America] Error al comunicarse con la fuente oficial powerball.com tras 3 intentos.")
        ...
        if fuente_info and db_ultimo:
            ...
            if fecha_fuente <= fecha_db:
                print(f"\nℹ️ [DETECCIÓN TEMPRANA] No hay sorteos nuevos para Lotto America.")
        ...
        if df_combined.empty:
            raise RuntimeError("❌ [Lotto America] No se lograron recuperar registros de la fuente ni de la BD.")
