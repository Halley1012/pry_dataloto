...
class EurojackpotPredictor:
    def __init__(self):
        self.max_white_ball = 50
        self.max_special_ball = 12
        self.features = ['Año', 'Mes', 'Día', 'dia_semana']
        self.loteria_route = 'eurojackpot'

    def run(self):
        print("🔮 Iniciando Predictor de Eurojackpot (Alemania - XGBoost)...")
        engine = get_engine()

        try:
            with engine.connect() as conn:
                df_all = pd.read_sql(
                    text('SELECT "fecha", "balota1", "balota2", "balota3", "balota4", "balota5", "balotaroja", "balotaroja2" '
                         'FROM resultados_eurojackpot '
                         'ORDER BY fecha ASC;'), conn
                )
        except Exception as e:
            print(f"❌ Error consultando resultados Eurojackpot: {e}")
            return

        if df_all.empty:
            print("⚠️ No hay resultados en la base de datos para entrenar el modelo de Eurojackpot.")
            return

        df_all["fecha"] = pd.to_datetime(df_all["fecha"])
        fecha_proximo_sorteo = df_all['fecha'].max().strftime('%Y-%m-%d')
        print(f"Próximo sorteo a predecir para Eurojackpot: {fecha_proximo_sorteo}")

        df3 = df_all[df_all['balota1'] > 0].copy().reset_index(drop=True)
        if df3.empty or len(df3) < 10:
            print("⚠️ No hay suficientes resultados históricos reales para entrenar Eurojackpot.")
            return
        ...
        # --- Crear columnas binarias para los 50 números regulares ---
        ...
        # --- Crear columnas binarias para las 12 Eurozahlen ---
        for s in range(1, self.max_special_ball + 1):
            df3[f's_{s}'] = df3[['balotaroja', 'balotaroja2']].apply(
                lambda row: int(s in row.values), axis=1
            )

        df_future = df_all[df_all['fecha'] == df_all['fecha'].max()].copy()
        if df_future.empty:
            print("⚠️ No se encontró la fila con la fecha reciente en Eurojackpot.")
            return
        ...
        # --- MODELO 1: 5 Números Regulares (1 al 50) ---
        ...
        # --- MODELO 2: 2 Eurozahlen (1 al 12) ---
        predicciones_estrellas = {}
        for s in range(1, self.max_special_ball + 1):
            target_col = f's_{s}'
            y_train_s = df_train[target_col]

            if y_train_s.nunique() < 2:
                predicciones_estrellas[s] = 0.05
                continue

            X_train_s = df_train[self.features]

            model_star = XGBClassifier(
                n_estimators=80,
                learning_rate=0.1,
                max_depth=1,
                subsample=0.8,
            ...
            model_star.fit(X_train_s, y_train_s)
            proba_s = model_star.predict_proba(X_future)[0, 1]
            predicciones_estrellas[s] = proba_s

        df_pred_star = pd.DataFrame.from_dict(predicciones_estrellas, orient='index', columns=['Probabilidad'])
        df_pred_star = df_pred_star.sort_values('Probabilidad', ascending=False)
        rojaprediccion = df_pred_star.index.tolist()
        ...
        try:
            with engine.begin() as conn:
                ...
                # Limpieza de predicciones viejas
                conn.execute(text("""
                    DELETE FROM predicciones 
                    WHERE LOWER(loteria_route) = :route 
                      AND fecha < CURRENT_DATE - INTERVAL '15 days';
                """), {"route": self.loteria_route})

                conn.execute(text("""
                    INSERT INTO predicciones (loteria_id, loteria_route, fecha, numeros, balotaroja)
                    VALUES (
                        (SELECT id FROM loterias WHERE LOWER(route) = :route OR LOWER(nombre) = 'eurojackpot' LIMIT 1),
                        :route,
                        :fecha,
                        :numeros,
                        :balotaroja
            ...
            print(f"✅ Predicción de Eurojackpot guardada exitosamente en BD para la fecha: {fecha_proximo_sorteo}")
            print(f"   Top Números: {numeros_prediccion[:5]} | Top Eurozahlen: {rojaprediccion[:2]}")
        except Exception as e:
            print(f"❌ Error guardando predicción de Eurojackpot en BD: {e}")
