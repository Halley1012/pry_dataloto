...
class ChispazoPredictor:
    ...
    def run(self):
        print("🔮 Iniciando Predictor de Chispazo (México - XGBoost)...")
        df = self.cargar_datos()

        if df.empty or len(df) < 15:
            print("⚠️ Insuficientes datos históricos para entrenar el modelo de Chispazo.")
            return
        ...
        # -------------------------------------------------------------
        # 1. Preparar Matriz de Balotas Principales (1 a 28)
        # -------------------------------------------------------------
        binary_dict = {}
        ...
        # Usar los últimos 1500 sorteos para entrenamiento
        train_idx = df_real.index[-1500:] if len(df_real) > 1500 else df_real.index[3:]
        X_train = df_features.iloc[train_idx]

        # Entrenar clasificadores para cada una de las 28 balotas naturales
        probabilidades = {}
        for i in range(1, self.max_balota + 1):
            target_col = f'b_{i}'
        ...
        try:
            with self.engine.connect() as conn:
                lot_row = conn.execute(text("SELECT id FROM loterias WHERE LOWER(route) = 'chispazo' LIMIT 1")).fetchone()
                if lot_row:
            ...
            print(f"✅ Predicción de Chispazo guardada exitosamente en BD para la fecha: {proxima_fecha}")
            print(f"Top 14 números con mayor probabilidad (del 1 al 28): {top_numeros[:14]}")

        except Exception as e:
            print(f"❌ Error guardando predicción de Chispazo en base de datos: {e}")

if __name__ == "__main__":
    predictor = ChispazoPredictor()
    predictor.run()
