...
class MelatePredictor:
    def __init__(self):
        self.engine = get_engine()
        self.max_balota = 56
        self.max_adicional = 56
        self.loteria_id = 16
        self.loteria_route = 'melate'

    def cargar_datos(self) -> pd.DataFrame:
        query = "SELECT * FROM resultados_melate ORDER BY fecha ASC;"
        with self.engine.connect() as conn:
            df = pd.read_sql(text(query), conn)
        return df

    def run(self):
        print("🔮 Iniciando Predictor de Melate (México - XGBoost)...")
        df = self.cargar_datos()

        if df.empty or len(df) < 15:
            print("⚠️ Insuficientes datos históricos para entrenar el modelo de Melate.")
            return
        ...
        print(f"Próximo sorteo a predecir para Melate: {proxima_fecha}")

        # Separar histórico real específico de Melate
        df_real = df[(df['sorteo'] == 'Melate') & (df['balota1'] > 0)].copy().reset_index(drop=True)
        if len(df_real) < 10:
            print("⚠️ Insuficientes sorteos reales para entrenar.")
            return

        # -------------------------------------------------------------
        # 1. Preparar Matriz de Balotas Principales (1 a 56)
        # -------------------------------------------------------------
        binary_dict = {}
        ...
        # Usar los últimos 1000 sorteos para entrenamiento rápido y de alta calidad
        train_idx = df_real.index[-1000:] if len(df_real) > 1000 else df_real.index[3:]
        X_train = df_features.iloc[train_idx]

        # Entrenar clasificadores para cada una de las 56 balotas naturales
        probabilidades = {}
        for i in range(1, self.max_balota + 1):
            target_col = f'b_{i}'
        ...
        # -------------------------------------------------------------
        # 2. Preparar Modelo para Número Adicional (1 a 56)
        # -------------------------------------------------------------
        binary_add_dict = {}
        ...
        try:
            with self.engine.connect() as conn:
                lot_row = conn.execute(text("SELECT id FROM loterias WHERE LOWER(route) = 'melate' LIMIT 1")).fetchone()
                if lot_row:
            ...
            print(f"✅ Predicción de Melate guardada exitosamente en BD para la fecha: {proxima_fecha}")
            ...
            print(f"❌ Error guardando predicción de Melate en base de datos: {e}")

if __name__ == "__main__":
    predictor = MelatePredictor()
    predictor.run()
