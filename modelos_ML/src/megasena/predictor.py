...
class MegaSenaPredictor:
    def __init__(self):
        ...
        self.max_balota = 60
        self.loteria_id = 21
        self.loteria_route = 'megasena'

    def cargar_datos(self) -> pd.DataFrame:
        query = "SELECT * FROM resultados_megasena ORDER BY fecha ASC;"
        with self.engine.connect() as conn:
            df = pd.read_sql(text(query), conn)
        return df

    def run(self):
        print("🔮 Iniciando Predictor de Mega-Sena (Brasil - XGBoost)...")
        df = self.cargar_datos()

        if df.empty or len(df) < 15:
            print("⚠️ Insuficientes datos históricos para entrenar el modelo de Mega-Sena.")
            return
        ...
        print(f"Próximo sorteo a predecir para Mega-Sena: {proxima_fecha}")
        ...
        # -------------------------------------------------------------
        # 1. Preparar Matriz de Balotas Principales (1 a 60)
        # -------------------------------------------------------------
        binary_dict = {}
        ...
        # Ingeniería de características construyendo diccionario para evitar fragmentación
        feat_dict = {
        ...
        # Entrenar clasificadores para cada una de las 60 balotas
        probabilidades = {}
        for i in range(1, self.max_balota + 1):
            target_col = f'b_{i}'
            ...
            # Probabilidad de que caiga el número
            prob = model.predict_proba(X_pred)[0][1] if len(model.classes_) > 1 else 0.05
            probabilidades[i] = prob

        # Ranking de las 60 balotas ordenadas de mayor a menor probabilidad
        ...
        try:
            with self.engine.connect() as conn:
                # Obtener loteria_id dinámicamente si existe
                lot_row = conn.execute(text("SELECT id FROM loterias WHERE LOWER(route) = 'megasena' OR LOWER(nombre) LIKE '%mega%sena%' LIMIT 1")).fetchone()
                if lot_row:
            ...
            print(f"✅ Predicción de Mega-Sena guardada exitosamente en BD para la fecha: {proxima_fecha}")
            print(f"Top 15 números principales con mayor probabilidad: {top_numeros[:15]}")

        except Exception as e:
            print(f"❌ Error guardando predicción de Mega-Sena en base de datos: {e}")
