...
class Lotto6aus49Predictor:
    def __init__(self):
        self.max_white_ball = 49
        self.max_superzahl = 10  # 0 al 9
        self.features = ['Año', 'Mes', 'Día', 'dia_semana']
        self.loteria_route = 'lotto_6aus49'

    def run(self):
        print("🔮 Iniciando Predictor de Lotto 6aus49 (Alemania - XGBoost)...")
        engine = get_engine()

        try:
            with engine.connect() as conn:
                df_all = pd.read_sql(
                    text('SELECT "fecha", "balota1", "balota2", "balota3", "balota4", "balota5", "balota6", "balotaroja" '
                         'FROM resultados_lotto_6aus49 '
                         'ORDER BY fecha ASC;'), conn
                )
        except Exception as e:
            print(f"❌ Error consultando resultados Lotto 6aus49: {e}")
            return

        if df_all.empty:
            print("⚠️ No hay resultados en la base de datos para entrenar el modelo de Lotto 6aus49.")
            return

        df_all["fecha"] = pd.to_datetime(df_all["fecha"])
        fecha_proximo_sorteo = df_all['fecha'].max().strftime('%Y-%m-%d')
        print(f"Próximo sorteo a predecir para Lotto 6aus49: {fecha_proximo_sorteo}")

        df3 = df_all[df_all['balota1'] > 0].copy().reset_index(drop=True)
        if df3.empty or len(df3) < 10:
            print("⚠️ No hay suficientes resultados históricos reales para entrenar Lotto 6aus49.")
            return
        ...
        # --- Crear columnas binarias para los 10 Superzahlen (0 al 9) ---
        for s in range(self.max_superzahl):
            df3[f'sz_{s}'] = (df3['balotaroja'] == s).astype(int)

        df_future = df_all[df_all['fecha'] == df_all['fecha'].max()].copy()
        if df_future.empty:
            print("⚠️ No se encontró la fila con la fecha reciente en Lotto 6aus49.")
            return
        ...
        # --- MODELO 2: Superzahl (0 al 9) ---
        predicciones_superzahl = {}
        for s in range(self.max_superzahl):
            target_col = f'sz_{s}'
            y_train_sz = df_train[target_col]

            if y_train_sz.nunique() < 2:
                predicciones_superzahl[s] = 0.05
                continue

            X_train_sz = df_train[self.features]

            model_sz = XGBClassifier(
                n_estimators=80,
                learning_rate=0.1,
                max_depth=1,
                subsample=0.8,
            ...
            model_sz.fit(X_train_sz, y_train_sz)
            proba_sz = model_sz.predict_proba(X_future)[0, 1]
            predicciones_superzahl[s] = proba_sz

        df_pred_sz = pd.DataFrame.from_dict(predicciones_superzahl, orient='index', columns=['Probabilidad'])
        df_pred_sz = df_pred_sz.sort_values('Probabilidad', ascending=False)
        superzahl_prediccion = df_pred_sz.index.tolist()
        ...
        try:
            with engine.begin() as conn:
                ...
                conn.execute(text("""
                    ...
                    VALUES (
                        (SELECT id FROM loterias WHERE LOWER(route) = :route OR LOWER(nombre) LIKE '%6aus49%' LIMIT 1),
                        :route,
                        :fecha,
                        :numeros,
                        :balotaroja
                ...
                """), {
                    ...
                    "balotaroja": superzahl_prediccion
                })

            print(f"✅ Predicción de Lotto 6aus49 guardada exitosamente en BD para la fecha: {fecha_proximo_sorteo}")
            print(f"   Top Números: {numeros_prediccion[:6]} | Top Superzahl: {superzahl_prediccion[:1]}")
        except Exception as e:
            print(f"❌ Error guardando predicción de Lotto 6aus49 en BD: {e}")
