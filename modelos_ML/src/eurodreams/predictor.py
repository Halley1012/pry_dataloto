...
class EuroDreamsPredictor:
    ...
    def run(self):
        ...
        df_features = pd.DataFrame(index=df_b.index)
        df_features['dia_semana'] = df_b['fecha'].dt.dayofweek
        df_features['mes'] = df_b['fecha'].dt.month
        df_features['dia'] = df_b['fecha'].dt.day

        # Lags y medias móviles de las balotas
        for lag in [1, 2, 3]:
            for col in cols_balotas:
                df_features[f'{col}_lag_{lag}'] = df_b[col].shift(lag)

        for w in [5, 10, 20]:
            for col in cols_balotas:
                df_features[f'{col}_roll_sum_{w}'] = df_b[col].shift(1).rolling(w, min_periods=1).sum()
        ...
        for d in range(1, self.max_dream + 1):
            df_d[f'd_{d}'] = (df_d['balotaroja'] == d).astype(int)

        cols_dream = [f'd_{d}' for d in range(1, self.max_dream + 1)]
        df_d_features = pd.DataFrame(index=df_d.index)
        df_d_features['dia_semana'] = df_d['fecha'].dt.dayofweek
        df_d_features['mes'] = df_d['fecha'].dt.month

        for lag in [1, 2, 3]:
            for col in cols_dream:
                df_d_features[f'{col}_lag_{lag}'] = df_d[col].shift(lag)

        for w in [5, 10]:
            for col in cols_dream:
                df_d_features[f'{col}_roll_sum_{w}'] = df_d[col].shift(1).rolling(w, min_periods=1).sum()

        df_d_features = df_d_features.fillna(0)
