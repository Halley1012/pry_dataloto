...
class CincoDeOroPredictor:
    ...
    def run(self):
        print("🔮 Iniciando Predictor de 5 de Oro y Revancha (Uruguay - XGBoost)...")
        df = self.cargar_datos()

        if df.empty or len(df) < 15:
            print("⚠️ Insuficientes datos históricos para entrenar el modelo de 5 de Oro.")
            return
        ...
        print(f"Próximo sorteo a predecir para 5 de Oro: {proxima_fecha}")
        ...
        # -------------------------------------------------------------
        # 1. Matriz de Balotas Principales (Pozo de Oro 1 a 48)
        # -------------------------------------------------------------
        binary_dict = {}
        ...
        # -------------------------------------------------------------
        # 2. Matriz y Modelo de Bolilla Extra (1 a 48)
        # -------------------------------------------------------------
        prob_extra = {}
        for i in range(1, self.max_extra + 1):
            y_ex = (df_real.iloc[train_idx]['balotaroja'] == i).astype(int)
            if y_ex.nunique() < 2:
                prob_extra[i] = 0.02
                continue

            model_ex = XGBClassifier(
                n_estimators=40,
                max_depth=3,
                learning_rate=0.05,
                eval_metric='logloss',
            ...
            model_ex.fit(X_train, y_ex)
            prob_ex = model_ex.predict_proba(X_pred)[0][1] if len(model_ex.classes_) > 1 else 0.02
            prob_extra[i] = prob_ex

        df_pred_ex = pd.DataFrame(list(prob_extra.items()), columns=['numero', 'probabilidad'])
        df_pred_ex = df_pred_ex.sort_values(by='probabilidad', ascending=False).reset_index(drop=True)
        top_extras = df_pred_ex['numero'].tolist()
        ...
        try:
            with self.engine.connect() as conn:
                lot_row = conn.execute(text("SELECT id FROM loterias WHERE LOWER(route) = '5deoro' LIMIT 1")).fetchone()
                if lot_row:
                ...
                conn.execute(insert_query, {
                    ...
                    "fecha": proxima_fecha,
                    "numeros": top_numeros,
                    "balotaroja": top_extras
                })
                conn.commit()

            print(f"✅ Predicción de 5 de Oro guardada exitosamente en BD para la fecha: {proxima_fecha}")
            print(f"Top 24 números de Pozo de Oro más probables: {top_numeros[:24]}")
            print(f"Top 5 Bolillas Extra más probables: {top_extras[:5]}")

        except Exception as e:
            print(f"❌ Error guardando predicción de 5 de Oro en base de datos: {e}")

if __name__ == "__main__":
    predictor = CincoDeOroPredictor()
    predictor.run()
