...
class PrimitivaPredictor:
    ...
    def run(self):
        print("🔮 Iniciando Predictor de La Primitiva (España - XGBoost)...")
        engine = get_engine()
        ...
        try:
            ...
            print(f"❌ Error consultando resultados La Primitiva: {e}")
            return

        if df_all.empty:
            print("⚠️ No hay resultados en la base de datos para entrenar el modelo de La Primitiva.")
            return

        df_all["fecha"] = pd.to_datetime(df_all["fecha"])
        fecha_proximo_sorteo = df_all['fecha'].max().strftime('%Y-%m-%d')
        print(f"Próximo sorteo a predecir para La Primitiva: {fecha_proximo_sorteo}")
        ...
        if df_future.empty:
            print("⚠️ No se encontró la fila con la fecha reciente en La Primitiva.")
            return
        ...
        try:
            ...
            conn.execute(text("""
                ...
                WHERE LOWER(loteria_route) = 'primitiva' 
            ...
            conn.execute(text("""
                ...
                VALUES (
                    (SELECT id FROM loterias WHERE LOWER(route) = 'primitiva' OR LOWER(nombre) LIKE '%primitiva%' LIMIT 1),
                    'primitiva',
                    :fecha,
                    :numeros,
                    :balotaroja
            ...
            print(f"✅ Predicción de La Primitiva guardada exitosamente en BD para la fecha: {fecha_proximo_sorteo}")
            ...
            print(f"❌ Error al guardar predicciones de La Primitiva en BD: {e}")
        finally:
            conn.close()

if __name__ == "__main__":
    PrimitivaPredictor().run()
