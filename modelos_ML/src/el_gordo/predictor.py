...
class ElGordoPredictor:
    def __init__(self):
        self.max_white_ball = 54
        self.max_clave = 10 # 0 al 9
        self.features = ['Año', 'Mes', 'Día', 'dia_semana']

    def run(self):
        print("🔮 Iniciando Predictor de El Gordo de la Primitiva (España - XGBoost)...")
        engine = get_engine()
        ...
        try:
            ...
            print(f"❌ Error consultando resultados El Gordo: {e}")
            return

        if df_all.empty:
            print("⚠️ No hay resultados en la base de datos para entrenar el modelo de El Gordo.")
            return

        df_all["fecha"] = pd.to_datetime(df_all["fecha"])
        fecha_proximo_sorteo = df_all['fecha'].max().strftime('%Y-%m-%d')
        print(f"Próximo sorteo a predecir para El Gordo: {fecha_proximo_sorteo}")
        ...
        # --- Crear columnas binarias para los 54 números regulares ---
        ...
        # --- Crear columnas binarias para los 10 Números Clave (0 al 9) ---
        for c in range(self.max_clave):
            df3[f'c_{c}'] = (df3['balotaroja'] == c).astype(int)

        df_future = df_all[df_all['fecha'] == df_all['fecha'].max()].copy()
        if df_future.empty:
            print("⚠️ No se encontró la fila con la fecha reciente en El Gordo.")
            return
        ...
        # --- MODELO 1: 5 Números Regulares (1 al 54) ---
        ...
        # --- MODELO 2: Número Clave (0 al 9) ---
        predicciones_clave = {}
        for c in range(self.max_clave):
            target_col = f'c_{c}'
            y_train_c = df_train[target_col]

            if y_train_c.nunique() < 2:
                predicciones_clave[c] = 0.0
                continue

            X_train_c = df_train[self.features]
            ...
            proba_c = model_c.predict_proba(X_future)[0, 1]
            predicciones_clave[c] = proba_c

        df_pred_clave = pd.DataFrame.from_dict(predicciones_clave, orient='index', columns=['Probabilidad'])
        df_pred_clave = df_pred_clave.sort_values('Probabilidad', ascending=False)
        clave_prediccion = df_pred_clave.index.tolist()
        ...
        try:
            ...
            conn.execute(text("""
                ...
                WHERE LOWER(loteria_route) = 'el_gordo' 
            ...
            conn.execute(text("""
                ...
                VALUES (
                    (SELECT id FROM loterias WHERE LOWER(route) = 'el_gordo' OR LOWER(nombre) LIKE '%gordo%' LIMIT 1),
                    'el_gordo',
                    :fecha,
                    :numeros,
                    :balotaroja
            ...
            """), {"fecha": fecha_proximo_sorteo, "numeros": numeros_prediccion, "balotaroja": clave_prediccion})
            conn.commit()

            print(f"✅ Predicción de El Gordo guardada exitosamente en BD para la fecha: {fecha_proximo_sorteo}")
            print(f"Top 15 números principales con mayor probabilidad: {numeros_prediccion[:15]}")
            print(f"Ranking Número Clave propuesto: {clave_prediccion}")

        except Exception as e:
            print(f"❌ Error al guardar predicciones de El Gordo en BD: {e}")
        finally:
            conn.close()

if __name__ == "__main__":
    ElGordoPredictor().run()
