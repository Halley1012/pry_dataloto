...
class MillionaireLifePredictor:
    ...
    def run(self):
        print("🔮 Iniciando Predictor Mejorado de Millionaire for Life...")
        engine = get_engine()
        ...
        try:
            ...
            print(f"❌ Error consultando resultados Millionaire for Life: {e}")
            return

        if df_all.empty:
            print("⚠️ No hay resultados en la base de datos para entrenar el modelo de Millionaire for Life.")
            return

        df_all["fecha"] = pd.to_datetime(df_all["fecha"])
        fecha_proximo_sorteo = df_all['fecha'].max().strftime('%Y-%m-%d')
        print(f"Próximo sorteo a predecir para Millionaire for Life: {fecha_proximo_sorteo}")
        ...
        try:
            ...
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS predicciones_millionaire_life (
            ...
            print(f"✅ Predicción de Millionaire for Life guardada exitosamente para la fecha del próximo sorteo: {fecha_proximo_sorteo}")
            print(f"Top 20 números con mayor probabilidad: {numeros_prediccion[:20]}")
            print(f"Top Cash Ball propuesta: {rojaprediccion[:5]}")

        except Exception as e:
            print(f"❌ Error al guardar predicciones de Millionaire for Life en BD: {e}")
        finally:
            conn.close()
