...
class MillionaireLifeScraper:
    ...
    def _asegurar_placeholder(self, ultima_fecha_real: date):
        ...
        with self.engine.begin() as conn:
            conn.execute(text("""
                DELETE FROM resultados_millionaire_life
            ...
            conn.execute(text("""
                INSERT INTO resultados_millionaire_life (
                    ...
                    NULL, :loteria_id, 'Millionaire for Life', :fecha,
                    0, 0, 0, 0, 0, 0,
        ...
        print(f"🎯 Placeholder verificado para Millionaire for Life ({prox_fecha_str})")
    ...
    def run(self, max_pages=None):
        ...
        if fuente_info is None:
            raise RuntimeError("❌ [Millionaire for Life] Error al comunicarse con la fuente oficial powerball.com tras 3 intentos.")
        ...
        if fuente_info and db_ultimo:
            ...
            if fecha_fuente <= fecha_db:
                print(f"\nℹ️ [DETECCIÓN TEMPRANA] No hay sorteos nuevos para Millionaire for Life.")
                ...
                self.update_jackpot(self.engine, "millionaire_life", "$1,000 / Day", prox_f)
        ...
        if df_combined.empty:
            raise RuntimeError("❌ [Millionaire for Life] No se lograron recuperar registros de la fuente ni de la BD.")
        ...
        self.update_jackpot(self.engine, "millionaire_life", "$1,000 / Day", prox_date)
        return {
            "hubo_sorteo": True,
            "ultimo_sorteo": f"Fecha {ultima_fecha_real}",
            "proximo_esperado": f"Fecha {prox_date}"
        }
