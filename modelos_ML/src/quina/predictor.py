...
class QuinaPredictor:
    def __init__(self):
        self.engine = get_engine()
        self.max_balota = 80
        self.loteria_id = 20
        self.loteria_route = 'quina'

    def cargar_datos(self) -> pd.DataFrame:
        query = "SELECT * FROM resultados_quina ORDER BY fecha ASC;"
        with self.engine.connect() as conn:
            df = pd.read_sql(text(query), conn)
        return df

    def run(self):
        print("🔮 Iniciando Predictor de Quina (Brasil - XGBoost)...")
        df = self.cargar_datos()

        if df.empty or len(df) < 15:
            print("⚠️ Insuficientes datos históricos para entrenar el modelo de Quina.")
            return
        ...
        print(f"Próximo sorteo a predecir para Quina: {proxima_fecha}")
        ...
        # -------------------------------------------------------------
        # 1. Preparar Matriz de Balotas Principales (1 a 80)
        # -------------------------------------------------------------
        binary_dict = {}
        ...
        # Usar los últimos 600 sorteos para entrenamiento rápido y preciso
        train_idx = df_real.index[-600:] if len(df_real) > 600 else df_real.index[3:]
        X_train = df_features.iloc[train_idx]

        # Entrenar clasificadores para cada una de las 80 balotas naturales
        probabilidades = {}
        for i in range(1, self.max_balota + 1):
            target_col = f'b_{i}'
        ...
        try:
            with self.engine.connect() as conn:
                lot_row = conn.execute(text("SELECT id FROM loterias WHERE LOWER(route) = 'quina' LIMIT 1")).fetchone()
                if lot_row:
            ...
            print(f"✅ Predicción de Quina guardada exitosamente en BD para la fecha: {proxima_fecha}")
            print(f"Top 20 números con mayor probabilidad (del 1 al 80): {top_numeros[:20]}")

        except Exception as e:
            print(f"❌ Error guardando predicción de Quina en base de datos: {e}")

if __name__ == "__main__":
    predictor = QuinaPredictor()
    predictor.run()
