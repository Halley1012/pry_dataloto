...
class GanaDiarioPredictor:
    def __init__(self):
        self.engine = get_engine()
        self.max_balota = 35
        self.loteria_id = 33
        self.loteria_route = 'ganadiario'

    def cargar_datos(self) -> pd.DataFrame:
        query = "SELECT * FROM resultados_ganadiario ORDER BY fecha ASC;"
        with self.engine.connect() as conn:
            df = pd.read_sql(text(query), conn)
        return df

    def run(self):
        print("🔮 Iniciando Predictor de Gana Diario (Perú - XGBoost)...")
        df = self.cargar_datos()

        if df.empty or len(df) < 15:
            print("⚠️ Insuficientes datos históricos para entrenar el modelo de Gana Diario.")
            return
        ...
        print(f"Próximo sorteo a predecir para Gana Diario: {proxima_fecha}")
        ...
        # -------------------------------------------------------------
        # 1. Matriz de Balotas Principales (1 a 35)
        # -------------------------------------------------------------
        binary_dict = {}
        ...
        try:
            with self.engine.connect() as conn:
                lot_row = conn.execute(text("SELECT id FROM loterias WHERE LOWER(route) = 'ganadiario' LIMIT 1")).fetchone()
                if lot_row:
            ...
            print(f"✅ Predicción de Gana Diario guardada exitosamente en BD para la fecha: {proxima_fecha}")
            print(f"Top 18 números naturales más probables (1 al 35): {top_numeros[:18]}")

        except Exception as e:
            print(f"❌ Error guardando predicción de Gana Diario en base de datos: {e}")

if __name__ == "__main__":
    predictor = GanaDiarioPredictor()
    predictor.run()
