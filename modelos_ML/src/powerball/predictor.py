...
class PowerballPredictor:
    ...
    def run(self):
        print("🔮 Iniciando Predictor de Powerball (Modelo Fecha/XGBoost)...")
        engine = get_engine()
        ...
        if df_future.empty:
            print(f"⚠️ No se encontró la fila con la fecha reciente en Powerball.")
            return
        ...
        try:
            ...
            conn.execute(text("""
                ...
                WHERE LOWER(loteria_route) = 'powerball' 
            ...
            conn.execute(text("""
                ...
                VALUES (
                    (SELECT id FROM loterias WHERE LOWER(route) = 'powerball' OR LOWER(nombre) = 'powerball' LIMIT 1),
                    'powerball',
                    :fecha,
                    :numeros,
                    :balotaroja
            ...
            print(f"✅ Predicción de Powerball guardada exitosamente en la tabla unificada para la fecha: {fecha_proximo_sorteo}")
