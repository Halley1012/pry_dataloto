...
class PowerballPredictor:
    ...
    def run(self):
        print("🔮 Iniciando Predictor Mejorado de Powerball...")
        engine = get_engine()
        ...
        try:
            ...
            print(f"❌ Error consultando resultados Powerball: {e}")
            return

        if df_all.empty:
            print("⚠️ No hay resultados en la base de datos para entrenar el modelo de Powerball.")
            return

        df_all["fecha"] = pd.to_datetime(df_all["fecha"])
        fecha_proximo_sorteo = df_all['fecha'].max().strftime('%Y-%m-%d')
        print(f"Próximo sorteo a predecir para Powerball: {fecha_proximo_sorteo}")

        # Filtramos solo sorteos reales históricos (excluyendo la fila del próximo sorteo en ceros)
        ...
        t_idx = num_draws # Índice para las características del próximo sorteo
        ...
        def build_features_vectorized(col, max_k):
            ...
            # cumsum[k] = sum(col[:k])  →  ventana [k-w, k) = cumsum[k] - cumsum[max(0,k-w)]
            cumsum = np.zeros(max_k + 1, dtype=np.int32)
            cumsum[1:] = np.cumsum(col)

            # decay incremental: decay[k+1] = α*decay[k] + col[k]  →  O(k) total
            alpha = np.exp(-0.03)
            decay_arr = np.zeros(max_k + 1)
            for i in range(max_k):
                decay_arr[i + 1] = alpha * decay_arr[i] + col[i]

            # recency en un solo pase: último índice donde col==1 antes de k
        ...
        for n in range(1, self.max_white_ball + 1):
            ...
            # Features del punto a predecir usando cumsum/decay ya calculados
        ...
        # --- MODELO 2: Powerball (Balota Roja) ---
        ...
        try:
            ...
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS predicciones_powerball (
            ...
            print(f"✅ Predicción de Powerball guardada exitosamente para la fecha del próximo sorteo: {fecha_proximo_sorteo}")
            print(f"Top 20 números con mayor probabilidad: {numeros_prediccion[:20]}")
            print(f"Top Powerball propuesta: {rojaprediccion[:5]}")

        except Exception as e:
            print(f"❌ Error al guardar predicciones de Powerball en BD: {e}")
        finally:
            conn.close()

if __name__ == "__main__":
    PowerballPredictor().run()
