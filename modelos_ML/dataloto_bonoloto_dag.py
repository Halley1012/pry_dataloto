...
def ejecutar_bonoloto():
    from main_bonoloto import main as main_bonoloto
    main_bonoloto()
...
def enviar_notificacion_exito():
    ...
    msg["Subject"] = "DAG Bonoloto ejecutado exitosamente"
    msg["From"] = sender
    msg["To"] = receiver

    html = """
    <h3>Ejecución de Bonoloto (España) finalizada con éxito</h3>
    <p>El proceso diario de scraping, predicción y notificaciones en <b>main_bonoloto.py</b> concluyó correctamente.</p>
    """
...
def enviar_notificacion_error(context):
    ...
    html = f"""
    <h3>⚠️ Alerta de Ejecución en Airflow - Bonoloto</h3>
...
with DAG(
    ...
    catchup=False,
    tags=['dataloto', 'bonoloto', 'spain', 'ml']
) as dag:

    tarea_ejecutar_bonoloto = PythonOperator(
        task_id='ejecutar_scraping_y_prediccion_bonoloto',
        python_callable=ejecutar_bonoloto
    )
    ...
    tarea_ejecutar_bonoloto >> tarea_notificar_exito
