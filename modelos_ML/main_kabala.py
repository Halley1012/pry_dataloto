...
def main():
    parser = argparse.ArgumentParser(description="Orquestador de Tareas ML para Kábala (Perú)")
    parser.add_argument(
        "--task",
        type=str,
        default="all",
    ...
    print("==================================================")
    print(f"Iniciando orquestación: Lotería=Kábala | Tarea={task} | Backfill={backfill}")
    print("==================================================")

    if task in ["scrap", "all"]:
        try:
            scraper_inst = KabalaScraper()
            ...
            print(f"❌ Falló la tarea de scraping para Kábala: {e}")
            sys.exit(1)

    if task in ["predict", "all"]:
        try:
            predictor_inst = KabalaPredictor()
            predictor_inst.run()
        except Exception as e:
            print(f"❌ Falló la tarea de predicción para Kábala: {e}")
            sys.exit(1)

    if task in ["notify", "all"]:
        try:
            notif_inst = NotificationGenerator()
            notif_inst.run("kabala")
        except Exception as e:
            print(f"❌ Falló la tarea de notificaciones para Kábala: {e}")

    print("✅ ¡Proceso completado exitosamente para Kábala!")
