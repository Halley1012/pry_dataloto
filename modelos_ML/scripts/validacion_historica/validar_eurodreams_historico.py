...
def validar_eurodreams_historico():
    print("=" * 50)
    print("Iniciando Validación Histórica para EuroDreams (España / Europa)")
    print("Lotería ID: 29 | 6 balotas (1..40) + Sueño / Balota Roja (1..5)")
    print("=" * 50)

    engine = get_engine()
    with engine.connect() as conn:
        total = conn.execute(text("SELECT COUNT(*) FROM resultados_eurodreams")).scalar()
        reales = conn.execute(text("SELECT COUNT(*) FROM resultados_eurodreams WHERE balota1 > 0")).scalar()
        placeholders = conn.execute(text("SELECT COUNT(*) FROM resultados_eurodreams WHERE balota1 = 0")).scalar()
        ...
        invalid_balls = conn.execute(text("""
            SELECT fecha, balota1, balota2, balota3, balota4, balota5, balota6, balotaroja
            FROM resultados_eurodreams
            WHERE balota1 > 0 AND (
                balota1 NOT BETWEEN 1 AND 40 OR
                balota2 NOT BETWEEN 1 AND 40 OR
                balota3 NOT BETWEEN 1 AND 40 OR
                balota4 NOT BETWEEN 1 AND 40 OR
                balota5 NOT BETWEEN 1 AND 40 OR
                balota6 NOT BETWEEN 1 AND 40 OR
                balotaroja NOT BETWEEN 1 AND 5
            );
        """)).fetchall()
        if invalid_balls:
            ...
            print("✅ Todas las balotas están en rango [1..40] y Sueño en rango [1..5]")

        # Placeholders
        phs = conn.execute(text("""
            SELECT fecha, sorteo, balota1, balota2, balota3, balota4, balota5, balota6, balotaroja, concurso
            FROM resultados_eurodreams
            WHERE balota1 = 0
            ORDER BY fecha ASC;
        """)).fetchall()
        ...
        for ph in phs:
            print(f"  - ({ph[0]} - {ph[1]}): Balotas=[{ph[2]}, {ph[3]}, {ph[4]}, {ph[5]}, {ph[6]}, {ph[7]}] | Sueño={ph[8]} | Concurso={ph[9]}")
    ...
    if fuente:
        print(f"  Último en Fuente: {fuente['fecha']} - Balotas: {fuente['balotas']} Sueño: {fuente['sueno']}")
    ...
    print("\n" + "=" * 50)
    print("🎉 AUDITORÍA DE DATOS DE EURODREAMS COMPLETADA")
    print("=" * 50)
    return True

if __name__ == "__main__":
    validar_eurodreams_historico()
