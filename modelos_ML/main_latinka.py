...
def main():
    parser = argparse.ArgumentParser(description="Orquestador de Tareas ML para La Tinka (Perú)")
    parser.add_argument(
        "--task",
        type=str,
        default="all",
    ...
    print("==================================================")
    print(f"Iniciando orquestación: Lotería=La Tinka | Tarea={task} | Backfill={backfill}")
    print("==================================================")

    if task in ["scrap", "all"]:
        try:
            scraper_inst = LaTinkaScraper()
            ...
            print(f"❌ Falló la tarea de scraping para La Tinka: {e}")
            sys.exit(1)

    if task in ["predict", "all"]:
        try:
            predictor_inst = LaTinkaPredictor()
            predictor_inst.run()
        except Exception as e:
            print(f"❌ Falló la tarea de predicción para La Tinka: {e}")
            sys.exit(1)

    if task in ["notify", "all"]:
        try:
            notif_inst = NotificationGenerator()
            notif_inst.run("latinka")
        except Exception as e:
            print(f"❌ Falló la tarea de notificaciones para La Tinka: {e}")

    print("✅ ¡Proceso completado exitosamente para La Tinka!")
