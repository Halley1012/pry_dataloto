...
def ejecutar_ganadiario():
    from main_ganadiario import main as main_ganadiario
    main_ganadiario()
...
def enviar_notificacion_exito():
    ...
    msg["Subject"] = "DAG Gana Diario ejecutado exitosamente"
    msg["From"] = sender
    msg["To"] = receiver

    html = """
    <h3>Ejecución de Gana Diario (Perú) finalizada con éxito</h3>
    <p>El proceso de scraping, predicción y notificaciones en <b>main_ganadiario.py</b> concluyó correctamente.</p>
    """
...
def enviar_notificacion_error(context):
    ...
    html = f"""
    <h3>⚠️ Alerta de Ejecución en Airflow - Gana Diario</h3>
...
with DAG(
    ...
    tarea_ejecutar_ganadiario = PythonOperator(
        task_id='ejecutar_scraping_y_prediccion_ganadiario',
        python_callable=ejecutar_ganadiario
    )
    ...
    tarea_ejecutar_ganadiario >> tarea_notificar_exito
