...
def validar_lotto_6aus49():
    print("==================================================")
    print("Iniciando Validación Histórica para Lotto 6aus49 (Alemania)")
    print("6 balotas (1..49) + 1 Superzahl (0..9) | Sin Revancha")
    print("==================================================")

    engine = get_engine()
    scraper = Lotto6aus49Scraper()

    # 1. Auditoría de datos en base de datos
    try:
        with engine.connect() as conn:
            df = pd.read_sql(text("SELECT * FROM resultados_lotto_6aus49 ORDER BY fecha ASC;"), conn)
    except Exception as e:
        print(f"❌ Error leyendo tabla resultados_lotto_6aus49: {e}")
        return False

    if df.empty:
        print("❌ Error: resultados_lotto_6aus49 está vacía.")
        return False
    ...
    # 4. Validar rangos de balotas (1..49, 6 números únicos) y Superzahl (0..9)
    ...
    for idx, row in df_real.iterrows():
        ...
        sz = int(row['balotaroja'])
        if not (0 <= sz <= 9):
            errores_sz += 1

    if errores_rango > 0:
        print(f"❌ Sorteos con balotas fuera de rango 1..49: {errores_rango}")
    else:
        print("✅ Todas las balotas principales dentro del rango 1..49")
    ...
    if errores_sz > 0:
        print(f"❌ Sorteos con Superzahl fuera de rango 0..9: {errores_sz}")
    else:
        print("✅ Todas las Superzahl dentro del rango 0..9")
    ...
    print("\n✅ Auditoría de Lotto 6aus49 completada con éxito.")
    return True

if __name__ == "__main__":
    validar_lotto_6aus49()
