...
def ejecutar_lotto_fr():
    from main_lotto_fr import main as main_lotto_fr
    main_lotto_fr()
...
def enviar_notificacion_exito():
    ...
    msg["Subject"] = "DAG Loto Francia ejecutado exitosamente"
    msg["From"] = sender
    msg["To"] = receiver

    html = """
    <h3>Ejecución de Loto Francia (FDJ) finalizada con éxito</h3>
    <p>El proceso de scraping, predicción y notificaciones en <b>main_lotto_fr.py</b> concluyó correctamente.</p>
    """
...
def enviar_notificacion_error(context):
    ...
    html = f"""
    <h3>⚠️ Alerta de Ejecución en Airflow - Loto Francia</h3>
...
with DAG(
    ...
    tarea_ejecutar_lotto_fr = PythonOperator(
        task_id='ejecutar_scraping_y_prediccion_lotto_fr',
        python_callable=ejecutar_lotto_fr
    )
    ...
    tarea_ejecutar_lotto_fr >> tarea_notificar_exito
