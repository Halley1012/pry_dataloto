...
def main():
    parser = argparse.ArgumentParser(description="Orquestador de Tareas ML para Melate Retro (México)")
    parser.add_argument(
        "--task",
        type=str,
        default="all",
    ...
    print("==================================================")
    print(f"Iniciando orquestación: Lotería=Melate Retro | Tarea={task} | Backfill={backfill}")
    print("==================================================")

    if task in ["scrap", "all"]:
        try:
            scraper_inst = MelateRetroScraper()
            ...
            print(f"❌ Falló la tarea de scraping para Melate Retro: {e}")
            sys.exit(1)

    if task in ["predict", "all"]:
        try:
            predictor_inst = MelateRetroPredictor()
            predictor_inst.run()
        except Exception as e:
            print(f"❌ Falló la tarea de predicción para Melate Retro: {e}")
            sys.exit(1)

    if task in ["notify", "all"]:
        try:
            notif_inst = NotificationGenerator()
            notif_inst.run("melateretro")
        except Exception as e:
            print(f"❌ Falló la tarea de notificaciones para Melate Retro: {e}")

    print("✅ ¡Proceso completado exitosamente para Melate Retro!")
