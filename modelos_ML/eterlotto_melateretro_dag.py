...
def ejecutar_melateretro():
    from main_melateretro import main as main_melateretro
    main_melateretro()
...
def enviar_notificacion_exito():
    ...
    msg["Subject"] = "DAG Melate Retro ejecutado exitosamente"
    msg["From"] = sender
    msg["To"] = receiver

    html = """
    <h3>Ejecución de Melate Retro (México) finalizada con éxito</h3>
    <p>El proceso de scraping, predicción y notificaciones en <b>main_melateretro.py</b> concluyó correctamente.</p>
    """
...
def enviar_notificacion_error(context):
    ...
    html = f"""
    <h3>⚠️ Alerta de Ejecución en Airflow - Melate Retro</h3>
...
with DAG(
    ...
    tarea_ejecutar_melateretro = PythonOperator(
        task_id='ejecutar_scraping_y_prediccion_melateretro',
        python_callable=ejecutar_melateretro
    )
    ...
    tarea_ejecutar_melateretro >> tarea_notificar_exito
