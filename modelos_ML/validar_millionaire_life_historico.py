...
def validar_millionaire_life():
    print("==================================================")
    print("Iniciando Validación Histórica para Millionaire for Life")
    print("Lotería ID: 14 / 38 | 5 balotas (1..58) + 1 Lucky Ball (1..5)")
    print("==================================================")
    ...
    with engine.connect() as conn:
        df = pd.read_sql(text("SELECT * FROM resultados_millionaire_life ORDER BY fecha ASC;"), conn)

    if df.empty:
        print("❌ Error: resultados_millionaire_life está vacía.")
        sys.exit(1)
    ...
    for _, r in df_ph.iterrows():
        print(f"  - ({r['fecha']} - {r['sorteo']}): Balotas={[r[c] for c in balota_cols]} LuckyBall={r.get('balotaroja')}")

    # 6. Sincronización con fuente oficial
    print("\n➡️ Verificando sincronización con la fuente oficial (powerball.com - millionaire-life)...")
    try:
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
            "X-Requested-With": "XMLHttpRequest"
        }
        r = requests.get("https://www.powerball.com/es/sorteos-anteriores?gc=millionaire-life&pg=1", headers=headers, timeout=10)
    ...
    print("\n==================================================")
    print("✅ Validación de Millionaire for Life completada exitosamente!")
    print("==================================================")

if __name__ == "__main__":
    validar_millionaire_life()
