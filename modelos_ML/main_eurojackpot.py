...
def main():
    parser = argparse.ArgumentParser(description="Orquestador de Tareas ML para Eurojackpot (Alemania)")
    parser.add_argument(
        "--task",
        type=str,
        default="all",
    ...
    print("==================================================")
    print(f"Iniciando orquestación: Lotería=Eurojackpot | Tarea={task} | Backfill={backfill}")
    print("==================================================")

    if task in ["scrap", "all"]:
        try:
            scraper_inst = EurojackpotScraper()
            ...
            print(f"❌ Falló la tarea de scraping para Eurojackpot: {e}")
            sys.exit(1)

    if task in ["predict", "all"]:
        try:
            predictor_inst = EurojackpotPredictor()
            predictor_inst.run()
        except Exception as e:
            print(f"❌ Falló la tarea de predicción para Eurojackpot: {e}")
            sys.exit(1)

    if task in ["notify", "all"]:
        try:
            notif_inst = NotificationGenerator()
            notif_inst.run("eurojackpot")
        except Exception as e:
            print(f"❌ Falló la tarea de notificaciones para Eurojackpot: {e}")

    print("✅ ¡Proceso completado exitosamente para Eurojackpot!")
