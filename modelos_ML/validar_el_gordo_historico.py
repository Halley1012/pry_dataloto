...
def validar_el_gordo_historico():
    print("=" * 50)
    print("Iniciando Validación Histórica para El Gordo de la Primitiva (España)")
    print("Lotería ID: 28 | 5 balotas (1..54) + Clave / Reintegro (0..9)")
    print("=" * 50)

    engine = get_engine()
    with engine.connect() as conn:
        total = conn.execute(text("SELECT COUNT(*) FROM resultados_el_gordo")).scalar()
        reales = conn.execute(text("SELECT COUNT(*) FROM resultados_el_gordo WHERE balota1 > 0")).scalar()
        placeholders = conn.execute(text("SELECT COUNT(*) FROM resultados_el_gordo WHERE balota1 = 0")).scalar()
        ...
        invalid_balls = conn.execute(text("""
            ...
            FROM resultados_el_gordo
            WHERE balota1 > 0 AND (
                balota1 NOT BETWEEN 1 AND 54 OR
                balota2 NOT BETWEEN 1 AND 54 OR
                balota3 NOT BETWEEN 1 AND 54 OR
                balota4 NOT BETWEEN 1 AND 54 OR
                balota5 NOT BETWEEN 1 AND 54 OR
                balotaroja NOT BETWEEN 0 AND 9
            );
        """)).fetchall()
        if invalid_balls:
            ...
            print("✅ Todas las balotas están en rango [1..54] y clave [0..9]")
        ...
        for ph in phs:
            print(f"  - ({ph[0]} - {ph[1]}): Balotas=[{ph[2]}, {ph[3]}, {ph[4]}, {ph[5]}, {ph[6]}] | Clave={ph[7]} | Concurso={ph[8]}")

    # Verificar con la fuente
    print("\n➡️ Verificando sincronización con la fuente (elgordodelaprimitiva.com.es)...")
    scraper = ElGordoScraper()
    fuente = scraper.extraer_ultimo_sorteo_fuente()
    if fuente:
        print(f"  Último en Fuente: {fuente['fecha']} - Balotas: {fuente['balotas']} Clave: {fuente['clave']}")
    ...
    print("\n" + "=" * 50)
    print("🎉 AUDITORÍA DE DATOS DE EL GORDO COMPLETADA")
    print("=" * 50)
    return True

if __name__ == "__main__":
    validar_el_gordo_historico()
